...
class Usuario_DAO {
    ...
    public function editUser($idUsuario, $nombre, $apellido, $correo_electronico, $habilitado, $perfil){
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $sql = $pdo->prepare("UPDATE usuarios set NOMBRE =:NOMBRE, APELLIDO =:APELLIDO, CORREO_ELECTRONICO =:CORREO_ELECTRONICO, HABILITADO =:HABILITADO, ID_PERFIL =:ID_PERFIL WHERE ID_USUARIO=:ID_USUARIO");

        if ($sql->execute(array('NOMBRE' => $nombre, 'APELLIDO' => $apellido, 'CORREO_ELECTRONICO' => $correo_electronico, 'HABILITADO' => $habilitado, 'ID_PERFIL' => $perfil, 'ID_USUARIO' => $idUsuario ))) {
            $status  = "success";
    ...
    public function newUser($nombre, $apellido, $email, $user_skype){
        ...
        if(!$this->isEmail($email)) {
            ...
            if ($resultado == null) {
                ...
                $sql = $pdo->prepare("INSERT INTO `usuarios`(`NOMBRE`, `APELLIDO`, `CORREO_ELECTRONICO`, `CLAVE_USUARIO`, `HABILITADO`, `ID_PERFIL`) VALUES (:nombre,:apellido,:correo,:password,'true','1')");
                $sql->execute(array('nombre' => $nombre, 'apellido' => $apellido, 'correo' => $email, 'password' => md5($password)));
                $id = $pdo->lastInsertId();
